...
struct ImageItem: Identifiable, Equatable {
    ...
    var dateExplanation: String? = nil
    var locationExplanation: String? = nil
...
@MainActor class AppSettings: ObservableObject {
    ...
    init() {
        ...
        self.customPrompt = UserDefaults.standard.string(forKey: "pinmage_custom_prompt") ?? "Analyze this image. It may be a scanned page containing multiple photos or a single photo. Identify any written text (captions, notes, dates, place names) AND visually recognize landmarks, architecture, signs, geographic features, or any other visual clues to determine:\n1. The approximate or exact date when the photo(s) were taken.\n2. The location/place name (city, country, landmark, address, region) where the photo was taken, as specific as possible.\n\nPrioritize written text when available, but use visual recognition of landmarks and scenery as supporting evidence. Be as accurate as possible. Return null for date or place if completely unknown.\n\nIMPORTANT — Place Name Specificity:\n- The 'place' field MUST include the most specific recognizable place name (landmark, institution, building, park, neighborhood), followed by the city and country.\n- Example: 'Instituto Cumbres de Caracas, Caracas, Venezuela' NOT just 'Caracas'.\n- Example: 'Eiffel Tower, Paris, France' NOT just 'Paris, France'.\n- Use canonical, well-known names that map services can find.\n\nIMPORTANT — Date formatting:\n- Only write the clean date (in YYYY-MM-DD, YYYY-MM, or YYYY format) in the 'date' field. If a specific day of the month is visible or written (e.g., '23 Julio 2000'), you MUST extract that exact day (e.g., '2000-07-23'). Do NOT default to the first of the month (e.g., '2000-07-01') if the specific day is known. Do NOT write descriptions, explanations, guesses, or parentheses in the 'date' field itself. Keep it strictly formatted.\n- Write your explanation, reasoning, or clues used to determine the date in the 'dateExplanation' field. ONLY date-related reasoning belongs here.\n\nIMPORTANT — Location Explanation:\n- Write your explanation, reasoning, or clues used to determine the location in the 'locationExplanation' field. ONLY location-related reasoning belongs here.\n- Do NOT mix date reasoning into locationExplanation or location reasoning into dateExplanation.\n\nIMPORTANT — Coordinates (latitude/longitude):\n- If you can identify the location, also provide approximate latitude and longitude in decimal degrees.\n- These are used as a fallback when geocoding services cannot find the place name.\n- Only provide coordinates if you are reasonably confident about the location.\n\nIMPORTANT certainty calibration guidelines:\n- Only report certainty above 90% if the date or location is explicitly and clearly visible in the image content (e.g., a handwritten caption, a printed date stamp from film development in the corner of a photo, a prominent sign).\n- If you are inferring from context clues, clothing, cars, or other indirect evidence, report lower certainty (30-70%).\n- Printed date stamps in the corner of old photos ARE part of the original photo content — treat them as high certainty.\n- Digital watermarks, copyright overlays, software branding logos, or text that appears digitally overlaid/post-processed rather than physically printed on the photo should be ignored.\n- If uncertain, report lower certainty rather than guessing confidently.\n\nCRITICAL — dateCertainty and locationCertainty values MUST be integers between 0 and 100 ONLY. Values like 95, 80, 50 are valid. A value of 500, 1000, or 0.95 is INVALID and will be rejected. NEVER return certainty outside 0-100."
